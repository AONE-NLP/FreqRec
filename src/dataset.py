...
def get_dataloder(args,seq_dic,fre=1.0):

    train_dataset = RecDataset(args, seq_dic['user_seq'], data_type='train')

    import random
    total_data = len(train_dataset)
    sample_size = int(total_data * fre)  # 计算10%的数据量
    random_indices = random.sample(range(total_data), sample_size)
    sampled_train_dataset = [train_dataset[i] for i in random_indices]
    train_sampler = RandomSampler(sampled_train_dataset)
